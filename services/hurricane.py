...
class Hurricane():
    ...
    def _poll_hurricane_info(self) -> tuple[str, str]:
        ...
        discussion_text.replace("Gulf of America", "Gulf of Mexico")
    ...
    def time_to_post_hurricane(self) -> tuple[bool, str, str]: # Determine if the timing is right to post hurricane information.
        ...
        if not 6 <= datetime.now().month <= 11:
            print("It is not hurricane season.")
    ...
    def time_to_post_preparedness(self) -> bool:
        currentTime = datetime.now().time()
        
        if datetime.now().month == 5 and datetime.now().day == 1:
            if self.prepardness_message["Start"] <= currentTime <= self.prepardness_message["End"] and not self.prepardness_message["sent"]:
                self.prepardness_message["sent"] = True
                return True
    ...
    def reset_states(self):
        ...
        self.prepardness_message["sent"] = False