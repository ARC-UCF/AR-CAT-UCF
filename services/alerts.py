...
class Alerts():
        ...
        if aList:
            for alert in aList: # Begin checking alerts.
                ...
                if alert["id"] not in self.ActiveAlerts: # If we don't have the alert, add it.
                    ...
                    similar, refer = self._check_for_similar(alert)
                    
                    if similar and refer in self.ActiveAlerts:
                        log.info("Alert is similar to an existing alert.")
                        alert["trackId"] = self.ActiveAlerts[refer]["trackId"]
                        alert["polyColor"] = self.ActiveAlerts[refer]["polyColor"]
                        alert["ignore"] = True
                    ...
                    if ref:
                        log.info("Alert is a reference to another alert.")
                        
                    if replacement:
                        log.info("Alert is a replacement of another alert.")
                        
                    if refer:
                        log.info("Similar alert referenced.")
                        
                    if not similar and not replacement and not ref:
    ...
    def _check_for_similar(self, alert: dict) -> tuple[bool, str]:
        ...
        if alert["title"] is None or alert["desc"] is None:
            log.info("Alert is mising title or description, skipping similarity check.")
            return False, None
        ...
        for key, info in self.ActiveAlerts.items():
            ...
            if info["title"] is None or info["desc"] is None:
                continue
